...
<body>
	<div class="mybody">
		...
		<form id="f_form" action="${base}/admin/dobanner" method="post" enctype="multipart/form-data">
		<h2 class="bg1">添加Banner</h2>
		...
		<h2 class="bg1">Banner列表</h2>
		[#list list as item]
		<div class="bg${item_index % 2 + 1}"><label><a href="${base}/admin/dobanner/${item.id}?do=del">删除</a></label>
		<span class="red">排序值:</span>${item.sortorder}
		<span class="red">名称:</span>${item.title}
		<span class="red">地址:</span>${item.originurl}
		<a href="${baseimg}${item.originalimg}" target="_blank"><img src="${baseimg}${item.titleimg}" style="width:90px; height:50px;" alt="${item.title}" title="title"></a>
		</div>
		[/#list]
